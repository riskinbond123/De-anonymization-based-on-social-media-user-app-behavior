...
x_test_data = np.loadtxt('x_test(501-18928)-user.txt')
y_test_data = np.loadtxt('y_test(501-18928)-app.txt')
...
for i in range(1,501):
    for j in range (1,7):
        temp = choice([k for k in range(1,501) if k not in [i]])
        combins_train.append([i,temp])
combins_train = np.asarray(combins_train)




label_train = np.zeros(3000) #comb(500, 2)
