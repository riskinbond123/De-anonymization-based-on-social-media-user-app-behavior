...
import numpy as np
import pandas as pd
data = pd.read_csv('output_lcc_graph.txt', sep = " ", header = None) # Reading the original file
...
# running col1.size and col2.size gives ________. These are the total nodes.
# running np.unique(col1).size and np.unique(col2).size gives 18950. These are the unique nodes which are returned sorted.
